...
const CSS = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  @keyframes terminalCursor { 0%,100%{opacity:1} 50%{opacity:0} }
  @keyframes glitchBlink {
    0%,93%,100%{opacity:1;text-shadow:0 0 10px #facc15}
    94%{opacity:.15;text-shadow:6px 0 #facc15}
    97%{opacity:.75;text-shadow:-4px 0 #facc15}
  }
  @keyframes scanline { 0%{transform:translateY(-100%)} 100%{transform:translateY(100vh)} }
  @keyframes ghostIn  { from{opacity:0;transform:translateX(-10px)} to{opacity:1;transform:translateX(0)} }
  @keyframes ghostOut { from{opacity:1;transform:translateX(0)} to{opacity:0;transform:translateX(6px)} }
  @keyframes spin     { to{transform:rotate(360deg)} }
  @keyframes successPulse {
    0%,100%{box-shadow:0 0 20px rgba(57,255,20,.25)}
    50%    {box-shadow:0 0 60px rgba(57,255,20,.55),0 0 100px rgba(57,255,20,.1)}
  }
  @keyframes warningPulse { 0%,100%{opacity:.55} 50%{opacity:1;text-shadow:0 0 8px #39ff14} }
  ...
  .ia-page::after {
    ...
    background-image:
      linear-gradient(rgba(57,255,20,.012) 1px,transparent 1px),
      linear-gradient(90deg,rgba(57,255,20,.012) 1px,transparent 1px);
    background-size:40px 40px;
    pointer-events:none; z-index:0;
  }

  /* ─── INNER WRAP — wider on landscape ─── */
  .ia-inner {
  ...
  .ia-tbar-title { flex:1; min-width:0; padding-left:4px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; }
  .ia-tbar-path  { margin-left:auto; opacity:0.3; font-size:clamp(0.45rem,1vw,0.55rem); white-space:nowrap; flex-shrink:0; }
  .ia-cursor { display:inline-block; width:6px; height:11px; background:#39ff14; margin-left:3px; animation:terminalCursor 1s step-end infinite; vertical-align:middle; flex-shrink:0; }
  ...
  .ia-header::before {
    ...
    background:linear-gradient(90deg,#facc15,rgba(57,255,20,.7),transparent);
    opacity:.45;
  }
  .ia-header::after {
    content:'';
    ...
    background:radial-gradient(circle,rgba(250,204,21,.05),transparent 70%);
    pointer-events:none;
  }
  .ia-header-tag { font-size:clamp(.52rem,1.2vw,.6rem); color:#39ff14; letter-spacing:.18em; text-transform:uppercase; margin-bottom:8px; animation:warningPulse 3s ease-in-out infinite; font-family:'Share Tech Mono', monospace; }
  .ia-title {
  ...
  .ia-title .yellow { color:#facc15; text-shadow:0 0 22px rgba(250,204,21,.4); }
  .ia-title .green  { color:#39ff14; text-shadow:0 0 22px rgba(57,255,20,.4); }

  .ia-subtitle { font-size:clamp(.62rem,1.5vw,.7rem); color:rgba(255,255,255,.45); letter-spacing:.04em; line-height:1.75; margin-top:6px; font-family:'Share Tech Mono', monospace; }
  .ia-req-star { color:#facc15; }

  .ia-badges { display:flex; gap:8px; flex-wrap:wrap; margin-top:14px; }
  .ia-badge {
    display:inline-flex; align-items:center; gap:4px;
    padding:3px 10px; border-radius:4px;
    font-size:clamp(.5rem,1.1vw,.58rem);
    letter-spacing:.08em; text-transform:uppercase;
    border:1px solid; white-space:nowrap;
    font-family:'Share Tech Mono', monospace;
  }
  .ia-badge.green  { color:#39ff14; border-color:rgba(57,255,20,.25);  background:rgba(57,255,20,.05);  }
  .ia-badge.yellow { color:#facc15; border-color:rgba(250,204,21,.25); background:rgba(250,204,21,.05); }
  .ia-badge.gray   { color:#94a3b8; border-color:rgba(148,163,184,.2); background:rgba(148,163,184,.04); }
  ...
  .ia-progress-label {
    ...
    font-size:clamp(.52rem,1.2vw,.6rem);
    color:rgba(57,255,20,.45); letter-spacing:.12em; margin-bottom:7px; text-transform:uppercase;
  }
  .ia-progress-track { height:2px; background:rgba(255,255,255,.05); border-radius:2px; overflow:hidden; }
  .ia-progress-fill  { height:100%; background:linear-gradient(90deg,#facc15,#39ff14); border-radius:2px; transition:width .4s ease; box-shadow:0 0 10px rgba(57,255,20,.4); }
  ...
  .ia-section:focus-within { border-color:rgba(250,204,21,.2); box-shadow:0 0 28px rgba(250,204,21,.03); }
  .ia-section::before {
    content:''; position:absolute; left:0; top:0; bottom:0; width:2px;
    background:linear-gradient(to bottom,#facc15,rgba(57,255,20,.5));
  ...
  .ia-section-label {
    ...
    font-size:clamp(.52rem,1.2vw,.58rem);
  ...
  /* ─── ROWS ─── */
  .ia-row { display:grid; grid-template-columns:1fr 1fr; gap:clamp(10px,2vw,14px); }
  .ia-row.single { grid-template-columns:1fr; max-width:320px; }
  .ia-group { display:flex; flex-direction:column; gap:6px; }
  .ia-label { font-size:clamp(.6rem,1.3vw,.65rem); color:rgba(255,255,255,.6); letter-spacing:.09em; text-transform:uppercase; }
  ...
  .ia-input::placeholder,.ia-textarea::placeholder { color:rgba(255,255,255,.2); }
  ...
  .ia-textarea { resize:vertical; min-height:clamp(100px,18vw,130px); line-height:1.75; }

  /* ─── HINTS & ERRORS ─── */
  .ia-hint  { font-size:clamp(.58rem,1.2vw,.63rem); color:rgba(57,255,20,.35); letter-spacing:.03em; line-height:1.5; }
  .ia-error { font-size:clamp(.6rem,1.3vw,.65rem); color:#f87171; letter-spacing:.03em; display:flex; align-items:center; gap:5px; flex-wrap:wrap; }
  .ia-error::before { content:'//'; color:rgba(248,113,113,.5); font-size:.6rem; flex-shrink:0; }
  ...
  .ia-file-inner {
    ...
    font-size:clamp(.7rem,1.8vw,.76rem);
    font-family:'Share Tech Mono',monospace;
    transition:all .25s;
    flex-wrap:wrap; word-break:break-all;
  }
  .ia-file-label:hover .ia-file-inner,
  .ia-file-label:focus-within .ia-file-inner { border-color:rgba(250,204,21,.5); background:rgba(250,204,21,.025); color:#facc15; }
  .ia-file-inner.err { border-color:rgba(248,113,113,.4); }
  ...
  .ia-check-wrap:hover { border-color:rgba(250,204,21,.2); background:rgba(250,204,21,.018); }
  .ia-check-wrap.err   { border-color:rgba(248,113,113,.35); }
  .ia-check-box { width:18px; height:18px; flex-shrink:0; margin-top:2px; border:1px solid rgba(250,204,21,.3); border-radius:3px; background:rgba(250,204,21,.04); position:relative; transition:all .2s; }
  .ia-check-box.checked { background:#facc15; border-color:#facc15; }
  .ia-check-box.checked::after { content:''; position:absolute; left:4px; top:1px; width:5px; height:10px; border:2px solid #000; border-top:none; border-left:none; transform:rotate(45deg); }
  .ia-check-text { font-size:clamp(.65rem,1.5vw,.72rem); color:rgba(255,255,255,.6); line-height:1.7; }
  ...
  .ia-btn-cancel:hover { border-color:rgba(255,255,255,.25); color:rgba(255,255,255,.6); }
  .ia-btn-cancel:disabled { opacity:.4; cursor:not-allowed; }
  ...
  .ia-btn-submit {
    ...
    letter-spacing:clamp(1px,.3vw,2px); text-transform:uppercase;
    border:none; border-radius:6px; cursor:pointer;
    transition:all .25s; display:flex; align-items:center; gap:8px; white-space:nowrap;
  }
  .ia-btn-submit:hover { background:#fde047; transform:translateY(-2px); box-shadow:0 6px 24px rgba(250,204,21,.4); }
  .ia-btn-submit:disabled { opacity:.5; cursor:not-allowed; transform:none; box-shadow:none; }

  .ia-spinner { width:13px; height:13px; border:2px solid rgba(0,0,0,.2); border-top-color:#000; border-radius:50%; animation:spin .7s linear infinite; flex-shrink:0; }
  ...
  .ia-global-err {
    ...
    border-radius:8px;
    font-size:.68rem; color:#fca5a5; letter-spacing:.03em;
  }

  /* ─── MATRIX CANVAS ─── */
  .ia-matrix-canvas { position:fixed; inset:0; opacity:.035; pointer-events:none; z-index:0; }
  ...
  .ia-success-card::before { content:''; position:absolute; top:0; left:0; right:0; height:2px; background:linear-gradient(90deg,transparent,#39ff14,transparent); opacity:.55; }
  .ia-success-icon { width:72px; height:72px; border-radius:50%; background:rgba(57,255,20,.08); border:1px solid rgba(57,255,20,.28); display:flex; align-items:center; justify-content:center; font-size:2rem; margin:0 auto 1.25rem; box-shadow:0 0 40px rgba(57,255,20,.15); }
  .ia-success-tag   { font-size:.58rem; color:#39ff14; letter-spacing:.22em; margin-bottom:8px; font-family:'Share Tech Mono', monospace; }
  .ia-success-title { font-family:'Orbitron',monospace; font-size:clamp(1.1rem,3vw,1.5rem); font-weight:900; color:#fff; letter-spacing:2px; text-transform:uppercase; margin-bottom:14px; animation:glitchBlink 6s infinite; }
  .ia-success-msg   { font-size:.7rem; color:rgba(255,255,255,.28); line-height:1.9; margin-bottom:22px; letter-spacing:.03em; }
  .ia-success-msg .g { color:rgba(57,255,20,.55); }
  .ia-success-ref   { display:inline-block; background:rgba(57,255,20,.05); border:1px solid rgba(57,255,20,.18); border-radius:5px; padding:8px 18px; font-size:.63rem; color:rgba(57,255,20,.45); letter-spacing:.15em; margin-bottom:28px; }
  .ia-success-btn   { padding:clamp(10px,2vw,12px) clamp(20px,5vw,32px); background:#facc15; color:#000; font-family:'Orbitron',monospace; font-weight:700; font-size:clamp(.62rem,1.5vw,.72rem); letter-spacing:2px; text-transform:uppercase; border:none; border-radius:6px; cursor:pointer; transition:all .25s; width:100%; max-width:280px; }
  .ia-success-btn:hover { background:#fde047; transform:translateY(-2px); box-shadow:0 6px 24px rgba(250,204,21,.4); }

  /* ══════════════════════════════════════════
     RESPONSIVE SYSTEM
  ══════════════════════════════════════════ */

  /* ── Landscape tablets & small laptops (≥860px) → expand inner wrap ── */
  @media (min-width: 860px) {
    .ia-inner { max-width: 1100px; }
    .ia-row   { gap: 20px; }
    .ia-row.single { max-width: 380px; }
  }

  /* ── Wide landscape (≥1200px) ── */
  @media (min-width: 1200px) {
    .ia-inner { max-width: 1320px; }
    /* Switch to 3-col for personal info row on ultra-wide */
    .ia-row.three { grid-template-columns: 1fr 1fr 1fr; }
  }

  /* ── Tablet portrait (≤768px) — keep 2-col but tighter ── */
  @media (max-width: 768px) and (orientation: portrait) {
    .ia-row { grid-template-columns: 1fr 1fr; gap: 12px; }
    .ia-row.single { max-width: 100%; }
  }

  /* ── Landscape mobile (phones in landscape) ── */
  @media (max-width: 900px) and (orientation: landscape) and (max-height: 500px) {
    .ia-page { padding-top: 3.5rem; padding-bottom: 2rem; }
    .ia-row  { grid-template-columns: 1fr 1fr; gap: 12px; }
    .ia-row.single { max-width: 340px; }
    .ia-inner { max-width: 98%; }
    .ia-tbar-path { display: none; }
  }

  /* ── Small portrait (≤600px) → collapse to 1-col ── */
  ...
  @media (max-width: 480px) {
    ...
    .ia-btn-cancel,.ia-btn-submit { width: 100%; justify-content: center; }
  ...
  @media (max-width: 360px) {
    .ia-page  { padding-left: .5rem; padding-right: .5rem; }
    .ia-tbar  { padding: .35rem .5rem; gap: 4px; }
    .ia-form-wrap { padding: .7rem .55rem; }
    .ia-section   { padding: .75rem .6rem; }
    .ia-title { font-size: .95rem; letter-spacing: .5px; }
  }
  ...
  return (
    <div className="ia-progress-wrap">
      <div className="ia-progress-label">
        ...
        <span style={{ color: pct === 100 ? "#39ff14" : "rgba(250,204,21,.5)" }}>{pct}%</span>
      </div>
...
const InternshipApplication = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!user) { navigate("/Login?redirect=/internship-application", { replace: true }); return; }
      ...
      <>
        ...
        <div className="ia-success-page">
          <motion.div
            ...
            transition={{ duration: .6, ease: [.34, 1.56, .64, 1] }}
          >
            <div className="ia-success-tag">// transmission_complete</div>
            <div className="ia-success-icon" style={{ color: "#39ff14", textShadow: "0 0 20px rgba(57,255,20,.8)" }}>✓</div>
            ...
            <br />
            <button className="ia-success-btn" onClick={() => navigate("/Career")}>← RETURN TO BASE</button>
          </motion.div>
        </div>
      </>
    );
    ...
    <>
      ...
      <div className="ia-page">
        <div className="ia-inner">
          ...
          <button className="ia-back" onClick={() => navigate("/Career")}>← BACK TO CAREERS</button>
          ...
          {/* Header */}
          <motion.div className="ia-header" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .45 }}>
          ...
          {/* Form body */}
          <motion.form className="ia-form-wrap" onSubmit={handleSubmit} noValidate
            initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .45, delay: .12 }}>

            <FormProgress formData={formData} />

            {errors._global && <div className="ia-global-err">// error: {errors._global}</div>}
            ...
            <Section num="5" label="psych_profile.query">
              ...
              >
                <textarea className={`ia-textarea${errors.motivation ? " err" : ""}`}
            ...
            <Section num="6" label="confirmation_protocol">
              <div className={`ia-check-wrap${errors.confirmed ? " err" : ""}`}
                onClick={() => {
